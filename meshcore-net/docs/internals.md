...
- `Core/Interfaces.cs`, `Core/CompanionLinks.cs`, `Core/CompanionRadioInterface.cs`, `Core/LoRaInterface.cs`, and `Core/EspNowInterface.cs` implement transport boundaries.
...
`EspNowInterface` wraps `IEspNowSocket`. The production implementation uses SharpPcap to open a monitor-mode WiFi interface, filter ESP-NOW action frames, decode plaintext vendor payloads, and inject broadcast frames. Encrypted ESP-NOW sending remains unsupported, matching the Python ESPythoNOW limitation.
...
- ESP-NOW defaults to device `wlan0`
...
3. Run hardware smoke checks for mock, WiFi companion, serial companion, LoRa, and ESP-NOW.
4. Fix any deltas.
